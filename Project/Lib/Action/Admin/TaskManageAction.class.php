...
// 				$data['time2'] = trim($time2);
 ...
 $strFilter and tb.autoid  not in ( select top $startNum tb1.autoid from t_task_base as tb1 ,t_position_mod as tm,t_unit as u,t_tool as t,t_task_stu as tu where tb.autoid=tu.taskid and t.id=tb1.tool and u.id=tb1.unit and tm.id=tb1.modelid $strFilter order by tb1.autoid desc ) order by tb.autoid desc";
		...
		$taskda = M()->table("t_task_base as tb")
			->where($filter)
			//->join("LEFT JOIN t_task_stu as tu ON tb.autoid = tu.taskid")
			->join("LEFT JOIN t_tool as t ON t.id = tb.tool")
			->join("LEFT JOIN t_unit as u ON u.id = tb.unit")
			->join("LEFT JOIN t_position_mod as tm ON tm.id = tb.modelid")
			...
			->field("tb.*, t.tipname as name1,u.unitname as name2,tm.tipname as name3")
			->order ( $order )
			->page ( $currentPage )->limit ( $pageSize )
			->select();

		foreach($taskda as $k=>&$v){
			$id = $v["autoid"];
			$task_stu = M()->table("t_task_stu")->where("taskid = $id")->find();
			//dump($task_stu);
			$v["mid"] = $task_stu["autoid"];
			$v["tret"] = $task_stu["ret"];
			$v["trettxt"] = $task_stu["rettxt"];
			$v["ttooltip"] = $task_stu["tooltip"];
		}
		//dump($taskda);
